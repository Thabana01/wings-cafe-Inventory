...
function ProductForm({ addProduct, editProduct, updateProduct }) {
  const [form, setForm] = useState({
    ...
    price: "",
    quantity: "",
    image: "" // new field for image filename
  });

  useEffect(() => {
    if (editProduct) setForm(editProduct);
    else setForm({ name: "", description: "", category: "", price: "", quantity: "", image: "" });
  }, [editProduct]);
  ...
  return (
    <form className="product-form" onSubmit={handleSubmit}>
      ...
      <input name="image" placeholder="Image filename (e.g. product1.jpg)" value={form.image} onChange={handleChange} />
